...
- Re-baseline the tested pin to Muse Code **1.3.0 (1.3.0-R3057.1)**
  (`muse-stable` channel rolled from 1.2.1). The echo fingerprint matches
  the snapshot, `muse --help` and `muse exec --help` are byte-identical to
  1.2.1, and the on-disk model catalog is unchanged (same `MuseModel` ids,
  default, and context/output limits). Pin-only release.
